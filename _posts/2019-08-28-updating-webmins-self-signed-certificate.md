...
It turned out the certificate had expired long ago but previous versions
of Webmin or the underlying libraries had not been detecting this fact.

The question was how to fix this. A number of answers posted on the Internet
suggested disabling SSL, logging in, and re-enabling it to generate a new
certificate. The problem is that if you log in with SSL disable, you expose
your password.

I tried finding the script in the Webmin code which generates
the certificate, but gave up after a while and found a command for generating a
self-signed certificate. Once I did that, I had to concatenate the private
encryption key and the self-signed certificate into the single file which
Webmin expects. Here are the commands:
